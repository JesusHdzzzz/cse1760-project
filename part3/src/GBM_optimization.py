...
def run_h2o_gbm(train, test, x_cols, y_col, seed=42) -> Dict[str, Any]:
    ...
    gbm = H2OGradientBoostingEstimator(
        ...
        # we can disable early stopping for base model here
        stopping_rounds=0,
...
# commented tests are ones that were run but did not yield better results
# compared to our base GBM model.
GBM_SWEEP_CONFIGS = [
    ...
    #},
    {
        "name": "GBM_n1100_d11_lr03", # best config found
        "ntrees": 1100,
        "max_depth": 11,
        "learn_rate": 0.03,
        "sample_rate": 0.8,
...
def run_gbm_sweep(train, test, x_cols, y_col, seed=42) -> List[Dict[str, Any]]:
    ...
    for cfg in GBM_SWEEP_CONFIGS:
        ...
        gbm = H2OGradientBoostingEstimator(
            ...
            nfolds=5,
            seed=seed,
            # early stopping disabled for sweep models, did not improve results
            stopping_rounds=0,
            stopping_metric="RMSE",
            stopping_tolerance=0.001,
            score_tree_interval=5,
