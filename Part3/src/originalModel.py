...
def plot_learning_curves(model, train, valid, metric='AUC'):
    """
    Plot learning curves to examine overfitting patterns
    """
    # Extract scoring history
    scoring_history = model.scoring_history().as_data_frame()
    
    # Get the number of trees/iterations
    if 'number_of_trees' in scoring_history.columns:
        x = scoring_history['number_of_trees']
    elif 'epochs' in scoring_history.columns:
        x = scoring_history['epochs']
    else:
        x = scoring_history.index
    
    plt.figure(figsize=(12, 5))
    
    # Plot training and validation metrics
    plt.subplot(1, 2, 1)
    if f'training_{metric.lower()}' in scoring_history.columns:
        plt.plot(x, scoring_history[f'training_{metric.lower()}'], 
                label=f'Train {metric}', linewidth=2, color='blue')
        plt.plot(x, scoring_history[f'validation_{metric.lower()}'], 
                label=f'Valid {metric}', linewidth=2, color='orange')
    else:
        # Try alternative column names
        for col in scoring_history.columns:
            if 'train' in col.lower() and metric.lower() in col.lower():
                plt.plot(x, scoring_history[col], label=f'Train {metric}', linewidth=2, color='blue')
            if 'valid' in col.lower() and metric.lower() in col.lower():
                plt.plot(x, scoring_history[col], label=f'Valid {metric}', linewidth=2, color='orange')
    
    plt.xlabel('Number of Trees/Iterations', fontsize=12)
    plt.ylabel(metric, fontsize=12)
    plt.title(f'{metric} Learning Curves', fontsize=14, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # Plot gap between train and validation
    plt.subplot(1, 2, 2)
    if f'training_{metric.lower()}' in scoring_history.columns:
        gap = scoring_history[f'training_{metric.lower()}'] - scoring_history[f'validation_{metric.lower()}']
        plt.plot(x, gap, label=f'Train-Valid Gap ({metric})', linewidth=2, color='red')
        plt.fill_between(x, 0, gap, alpha=0.3, color='red')
    ...
    plt.xlabel('Number of Trees/Iterations', fontsize=12)
    plt.ylabel(f'Train-Valid {metric} Gap', fontsize=12)
    plt.title(f'Overfitting Detection\n(Gap > 0.1 indicates overfitting)', fontsize=14, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
...
# Call it in both scripts
plot_learning_curves(best_gbm, train, valid, metric='AUC')
plt.show()


plot_learning_curves(best_gbm, train, valid, metric='AUC')
plt.savefig('second_script_learning_curves.png', dpi=300, bbox_inches='tight')
# --- 8. SHUTDOWN ---
